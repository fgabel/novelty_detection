...
"""
    Utils file for custom numpy routines
"""

def scatter_numpy(self, dim, index, src):
    """
    Credits: https://stackoverflow.com/users/8021282/sia-rezaei
...
# Testing the function scatter_numpy
"""
H, W, C = 2, 2, 3
src = np.ones((H,W,C))
binary_labels = np.zeros((H,W,C))
indices = np.array([
    [0, 1],
    [2, 2]
])
indices = np.repeat(indices[:, :, np.newaxis], C, axis=2)
binary_labels = scatter_numpy(binary_labels, 2, indices, src)
